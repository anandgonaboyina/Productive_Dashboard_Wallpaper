    ...
    if (body.action === 'check') {
      try {
        ...
        if (count > 0) {
          ...
          return NextResponse.json({ success: true, updateAvailable: false, message: 'Your dashboard is already up to date with latest features and improvements from GitHub.' });
        }
      } catch (err: any) {
