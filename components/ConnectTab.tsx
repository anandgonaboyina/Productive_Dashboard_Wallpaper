...
import { Users, UserPlus, Rss, LogIn, UserCircle, Search, Check, X, ShieldAlert, BarChart2 } from 'lucide-react';
...
export default function ConnectTab({ friendStats, setFriendStats }: ConnectTabProps) {
  ...
  const fetchRoadmap = async () => {
    try {
      ...
      if (res.ok) setRoadmapItems(data.roadmap || []);
    } catch (_) {}
  };
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem('dashboard_sync_token');
    try {
      const res = await fetch(`${apiUrl}/api/friends/search?q=${encodeURIComponent(searchQuery)}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
  ...
  const removeFriend = async (friendshipId: string) => {
    ...
    const token = localStorage.getItem('dashboard_sync_token');
    try {
      const res = await fetch(`${apiUrl}/api/friends?id=${friendshipId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) fetchFriendsData();
  ...
  const viewFriendStats = async (friendId: string, friendUsername: string) => {
    const token = localStorage.getItem('dashboard_sync_token');
    try {
      const res = await fetch(`${apiUrl}/api/friends/stats?friendId=${friendId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
      if (res.ok) {
